 ...
 */
  ...
  function getPossibleAnchors() {
      return new Promise(function(fulfill, reject) {
        ...
        request({uri: tr}, function(error, response, body) {
          ...
          var tocAnchors = $('#toc ol a');

          for (var key in tocAnchors) {
            if (tocAnchors.hasOwnProperty(key)) {
              var element = tocAnchors[key];

              if(element.attribs == undefined) continue;

              if(!anchors.includes(element.attribs.href)) {
                anchors.push(element.attribs.href);
              }
            }
          }
  ...
  */
    ...
    return new Promise(function(fulfill, reject) {
        for (var i = 0; i < results.length; i++) {
          if(!anchors.includes(results[i].name)) {
            ...
            if(results[i].name.indexOf('propdef') !== -1 ||
               results[i].name.indexOf('valdef') !== -1) continue;
  ...
  /*
    Anchors not in Results
    -------------------------------
    These are anchors that were found on
    the page that don't have any correlating
    tests.
  */
  function anchorsNotInResults() {
    return new Promise(function(fulfill, reject) {
        for (var i = 0; i < anchors.length; i++) {
          var el = _.find(results, {'name': anchors[i]});

          if(el == undefined) {
            anchorsMissing.push(anchors[i]);
          }
        }
        return fulfill();
      });
  ...
  */
  function printResults() {
    ...
    console.log(" ");
    console.log("Anchors not tested");
    console.log("------------------");
    for(var r = 0; r < anchorsMissing.length; r++) {
      console.log(anchorsMissing[r]);
    }
  ...
  /*
    ...
    If it does it will increase the count of usage.
  */
